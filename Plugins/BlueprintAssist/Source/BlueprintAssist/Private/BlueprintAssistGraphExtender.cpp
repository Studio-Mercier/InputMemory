...
void FBAGraphExtender::GenerateCreateEventNode(const UEdGraphPin* Pin)
{
	...
	TSharedPtr<FBAScopedGraphAction> Transaction = MakeShared<FBAScopedGraphAction>(GraphHandler, "Generate Create Event Node");
	...
	if (UK2Node_CreateDelegate* CreateEventNode = FEdGraphSchemaAction_K2NewNode::SpawnNode<UK2Node_CreateDelegate>(ParentGraph, GraphPosition, EK2NewNodeFlags::None))
	{
		...
		EBAAutoFormatting AutoFormatting = UBASettings::GetFormatterSettings(ParentGraph).GetAutoFormatting();
		if (AutoFormatting != EBAAutoFormatting::Never)
		{
			GraphHandler->RequestFormatNode(Node, Transaction);
			GraphHandler->RequestFormatNode(CreateEventNode, Transaction);
		}
	}
}
