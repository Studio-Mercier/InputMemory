...
void FBAGraphActions::CreateRerouteNode()
{
	...
	FPinLink HoveredLink = FBAUtils::GetHoveredPinLink(GraphPanel);
	if (HoveredLink.HasBothPins())
	{
		const FBAScopedGraphAction Transaction(GraphHandler, "Create Reroute Node On Wire");
		FBAUtils::CreateKnotNode(GraphHandler->GetFocusedEdGraph(), GraphPosition - NodeSpacerSize * 0.5f, HoveredLink.GetFromPin(), HoveredLink.GetToPin());
		return;
	}
	...
	if (HoveredGraphPin.IsValid())
	{
		...
		{
			const FBAScopedGraphAction Transaction(GraphHandler, "Link To Reroute Node");
	...
	{
		const FBAScopedGraphAction Transaction(GraphHandler, "Create Reroute Node");
