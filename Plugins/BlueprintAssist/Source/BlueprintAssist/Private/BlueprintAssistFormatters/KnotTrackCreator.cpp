...
void FKnotTrackCreator::MakeKnotTrack()
{
	...
	const FEdGraphFormatterParameters& FormatterParams = Formatter->GetFormatterParameters();

	const auto& NotFormatted = [FormattedNodes, FormatterParams](UEdGraphPin* Pin)
	{
		if (!Pin)
		{
			return true;
		...
		if (!FormatterParams.IsValidPin(Pin))
		{
			return true;
		}

		if (!FormattedNodes.Contains(Pin->GetOwningNode()))
		{
			return true;
		}
	...
	TSet<UEdGraphPin*> PinsRequiringTracks;
	...
	for (UEdGraphNode* MyNode : FormattedNodes)
	{
		...
		{
			if (!FormatterParams.IsValidPin(MyPin))
			{
				continue;
			}
			...
			{
				if (UEdGraphPin* LastPin = DoesNeedExecTrack(MyPin, LinkedTo))
				{
					PinsRequiringTracks.Add(MyPin);
					PinsRequiringTracks.Add(LastPin);
				}
			}
			else
			{
				...
				{
					if (UEdGraphPin* LastPin = DoesNeedExecTrack(MyPin, { Pin }))
					{
						PinsRequiringTracks.Add(MyPin);
						PinsRequiringTracks.Add(LastPin);
					}
		...
		for (UEdGraphPin* MyPin : FBAUtils::GetExecOrDelegatePins(MyNode, EGPD_Input))
		{
			...
			if (UBASettings::Get().ExecutionWiringStyle == EBAWiringStyle::AlwaysMerge)
			{
				MakeKnotTracksForLinkedExecPins(MyPin, LinkedTo, PinsRequiringTracks);
			}
			else
			{
				for (UEdGraphPin* Pin : LinkedTo)
				{
					MakeKnotTracksForLinkedExecPins(MyPin, { Pin }, PinsRequiringTracks);
				}
			}
		}
	}
...
UEdGraphPin* FKnotTrackCreator::DoesNeedExecTrack(UEdGraphPin* ParentPin, TArray<UEdGraphPin*> LinkedPins)
{
	...
	for (auto It = LinkedPins.CreateIterator(); It; ++It)
	{
		UEdGraphPin* LinkedPin = *It;
	...
	// remove pins which are left or too close to the parent pin
	const float Threshold = ParentPinPos.X - NodePadding.X * 1.5f;
	const auto IsTooCloseToParent = [&](UEdGraphPin* Pin)
	{
		const FVector2D PinPos = FBAUtils::GetPinPos(GraphHandler, Pin);
	...
	const bool bPinReallyFar = DistX >= UBASettings::Get().KnotNodeDistanceThreshold;

	const bool bPinNeedsTrack = DoesPinNeedTrack(ParentPin, LinkedPins);

	const bool bNeedsReroute = bPinNeedsTrack || bHeightDistance || bPinReallyFar;
	if (!bNeedsReroute)
	{
		return nullptr;
	}
	...
	return LinkedPins.Last();
}

TSharedPtr<FKnotNodeTrack> FKnotTrackCreator::MakeKnotTracksForLinkedExecPins(UEdGraphPin* ParentPin, TArray<UEdGraphPin*> LinkedPins, TSet<UEdGraphPin*>& PinsRequiringTracks)
{
	FVector2D ParentPinPos = FBAUtils::GetPinPos(GraphHandler, ParentPin);
	...
	UEdGraphPin* LastPin = LinkedPins.Last();
	const FVector2D LastPinPos = FBAUtils::GetPinPos(GraphHandler, LastPin);
	...
	const auto ByHighestPin = FBAUtils::GetHighestPinPredicate(GraphHandler);

	bool bPreviousParentNodeHasTrack = false;
	{
		TArray<UEdGraphPin*> ParentPinInDir = FBAUtils::GetLinkedPins(ParentPin->GetOwningNode(), ParentPin->Direction);
		ParentPinInDir.StableSort(ByHighestPin);
		for (int i = ParentPinInDir.Find(ParentPin) - 1; i >= 0; --i)
		{
			UEdGraphPin* Pin = ParentPinInDir[i];
			if (PinsRequiringTracks.Contains(Pin))
			{
				bPreviousParentNodeHasTrack = true;
				break;
			}
		}
	}

	bool bPreviousLastNodeHasTrack = false;
	{
		TArray<UEdGraphPin*> LastPinInDir = FBAUtils::GetLinkedPins(LastPin->GetOwningNode(), LastPin->Direction);
		LastPinInDir.StableSort(ByHighestPin);
		for (int i = LastPinInDir.Find(LastPin) - 1; i >= 0; --i)
		{
			UEdGraphPin* Pin = LastPinInDir[i];
			if (PinsRequiringTracks.Contains(Pin))
			{
				bPreviousLastNodeHasTrack = true;
				break;
			}
		}
	}

	if (!bPreviousLastNodeHasTrack)
	{
	...
	const bool bNeedsReroute = bPinNeedsTrack || bHeightDistance || bPreviousParentNodeHasTrack || bPinReallyFar;
	if (!bNeedsReroute)
	{
		return nullptr;
	}
	...
	// remove the first linked pins which has the same height and no collision
	const bool bSameHeightAsParentPin = FMath::Abs(KnotTrack->GetTrackHeight() - ParentPinPos.Y) < 5.f;
	if (!bSameHeightAsParentPin || LinkedPins.Num() == 1)
	{
		for (UEdGraphPin* LinkedPin : LinkedPins)
		{
			const FVector2D LinkedPinPos = FBAUtils::GetPinPos(GraphHandler, LinkedPin);
