...
FString FBAInputProcessor::GetCommandHistory()
{
	...
	for (const FName& Name : CommandHistory)
	{
		History += Name.ToString();
		History += TEXT(",");
	}

	History.RemoveFromEnd(TEXT(","));
			...
			if (Action)
			{
				if (Action->CanExecute() && (!KeyEvent.IsRepeat() || Action->CanRepeat()))
				{
					...
					if (!DisabledCommands.Contains(Command->GetCommandName()))
					{
						...
						const bool bWasExecuted = Action->Execute();
						if (bWasExecuted)
						{
							CommandHistory.Add(Command->GetCommandName());
							if (CommandHistory.Num() > 5)
							{
								CommandHistory.PopFront();
							}
						}
